...
import { UploadCloud, FileText, Loader2, RefreshCw, Download, ChevronDown, ChevronUp, Check, CreditCard, LogOut, Users, Trash2, Plus, Shield } from 'lucide-react';
...
export default function Invoices() {
    ...
    const canApprove = financeUser?.is_admin || financeUser?.can_approve;
    ...
    const handleApprove = async (id) => {
        try {
            const r = await fetch(`/api/invoices/${id}/approve`, { method: 'PUT', headers: { ...getFinanceHeaders() } });
    ...
    const handleMarkPaid = async (id) => {
        if (!paidByInput[id]?.trim()) return alert('Please enter who paid the bill');
        try {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const r = await fetch(`/api/invoices/${id}`, { method: 'DELETE', headers: { ...getFinanceHeaders() } });
            if (!r.ok) throw new Error((await r.json()).error);
            loadInvoices();
        } catch (err) { alert(err.message); }
    };
    ...
    const toggleApprover = async (userId, current) => {
        try {
            await fetch(`/api/finance/users/${userId}/approver`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json', ...getFinanceHeaders() },
                body: JSON.stringify({ can_approve: !current })
            });
            loadUsers();
        } catch (err) { alert(err.message); }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                    <span className="text-sm text-text-muted">
                        ...
                        {canApprove && <span className="ml-1 text-xs text-green-600">(Approver)</span>}
                    </span>
                    ...
                    <button onClick={loadInvoices} className="flex items-center gap-1.5 text-sm text-text-muted hover:text-text-main transition-colors">
                        <RefreshCw className="w-4 h-4" />
                    </button>
                    <button onClick={financeLogout} className="flex items-center gap-1.5 text-sm px-3 py-1.5 border border-red-200 text-red-600 rounded-lg hover:bg-red-50 transition-colors">
                        ...
                        <form onSubmit={addUser} className="flex items-end gap-3">
                            <div className="flex-1">
                                ...
                                <input type="text" value={newUser.username} onChange={e => setNewUser(p => ({ ...p, username: e.target.value }))}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary outline-none" required />
                            </div>
                            <div className="flex-1">
                            ...
                            {users.map(u => (
                                <div key={u.id} className="flex items-center justify-between py-2.5">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {u.can_approve && !u.is_admin && <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full font-medium">Approver</span>}
                                    </div>
                                    <div className="flex items-center gap-2">
                                        {!u.is_admin && (
                                            <>
                                                <button
                                                    onClick={() => toggleApprover(u.id, u.can_approve)}
                                                    className={`flex items-center gap-1 text-xs px-2.5 py-1 rounded-md border transition-colors ${u.can_approve ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100'}`}
                                                    title={u.can_approve ? 'Remove approver' : 'Make approver'}
                                                >
                                                    <Shield className="w-3 h-3" /> {u.can_approve ? 'Approver ✓' : 'Make Approver'}
                                                </button>
            ...
            <div className="bg-surface rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-6">
                    ...
                    {uploadStatus && <p className={`mt-3 text-sm ${uploadStatus.includes('Error') ? 'text-red-600' : 'text-green-600'}`}>{uploadStatus}</p>}
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-text-main">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? (
                                <tr><td colSpan="7" className="px-6 py-8 text-center text-text-muted"><Loader2 className="w-5 h-5 animate-spin inline mr-2" />Loading...</td></tr>
                            ) : invoices.length === 0 ? (
                                    ...
                                    return (
                                        <>
                                            ...
                                            <tr key={inv.id} className="hover:bg-gray-50/50 transition-colors">
                                                <td className="px-6 py-4 whitespace-nowrap text-text-muted">{new Date(inv.created_at).toLocaleDateString()}</td>
                                                <td className="px-6 py-4 font-medium max-w-[160px] truncate" title={inv.file_name}>{inv.file_name}</td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_STYLES[inv.status] || STATUS_STYLES.Pending}`}>{inv.status}</span>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        {/* Approve - admin/approver only, for Processed invoices */}
                                                        {inv.status === 'Processed' && canApprove && (
                                                        ...
                                                        {/* Paid by - for Approved invoices */}
                                                        {inv.status === 'Approved' && (
                                                            <div className="flex items-center gap-1.5">
                                                                <input type="text" placeholder="Who paid?" value={paidByInput[inv.id] || ''}
                                                                    onChange={e => setPaidByInput(p => ({ ...p, [inv.id]: e.target.value }))}
                                                        ...
                                                        {financeUser?.is_admin && (
                                                            <button onClick={() => handleDelete(inv.id)}
                                                                className="inline-flex items-center p-1.5 text-xs text-red-500 hover:text-red-700 hover:bg-red-50 rounded-md transition-colors">
                                            ...
                                            {isExpanded && inv.line_items?.length > 0 && (
                                                <tr key={`${inv.id}-details`}>
                                                    <td colSpan="7" className="px-6 py-4 bg-gray-50/80">
                                                        <h4 className="text-xs font-semibold uppercase text-text-muted mb-3">Line Items</h4>
